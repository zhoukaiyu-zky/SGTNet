...
class DAPPM(BaseModule):
    ...
    def __init__(self,
                 ...
                 num_scales: int,
                 kernel_sizes: List[int] = [3, 5, 7,1,3,5],
...
class RPPM(DAPPM):
    ...
    def __init__(self,
                 ...
                 out_channels: int,
                 num_scales: int,
                 kernel_sizes: List[int] = [5, 9, 17,1,1,1],
                 #kernel_sizes: List[int] = [3, 5, 7, 5, 3, 1],
                 strides: List[int] = [2, 4, 8],
                 paddings: List[int] = [2, 4, 8],
        ...
        self.deploy = deploy
        self.Conv1x1Layer1 = Block1x1(512, 128)
    ...
    def forward(self, inputs: Tensor):
        x_ = self.scales[0](inputs)
        #print(x_.size())#([6, 128, 32, 32])

        feats = []
        for i in range(1, self.num_scales):
            #s = self.scales[i](inputs)
            #print("xxxxxxxxxxxxxxxxxx", s.shape)
            feat_up = F.interpolate(
                self.scales[i](inputs),
                size=inputs.shape[2:],
                mode=self.unsample_mode,
            ...
            if feat_up.shape[1]>128:
                feat_up = self.Conv1x1Layer1(feat_up)
            #print(feat_up.size())  # ([6, 512, 32, 32])
            # print("xxxxxxxxxxxxxxxxxx", x_.shape)
            feats.append(feat_up + x_)
        scale_out = self.processes(torch.cat(feats, dim=1))
        return self.compression(torch.cat([x_, scale_out],
                                          dim=1)) + self.shortcut(inputs)
